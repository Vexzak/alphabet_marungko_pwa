...
function buildAllRounds(discoveredLetters: string[], allLetters: LetterProgress[]): ReviewRound[] {
  ...
  return shuffledLetters.map((lp) => {
    ...
    return { image: asset.image, sound: asset.sound, word: asset.word, correctLetter: correctDisplay, choices: shuffle([correctDisplay, ...wrong]) };
  });
}
...
function CircleCard({ label, isCorrect, onCircled, disabled, revealed, entryDelay = 0 }: {
  ...
  const spawnStars = useCallback((canvasEl: HTMLCanvasElement, x: number, y: number) => {
    const rect = canvasEl.getBoundingClientRect(); const hues = [50, 200, 280, 320];
    for (let i = 0; i < 10; i++) {
      const angle = Math.random() * Math.PI * 2, vel = 60 + Math.random() * 80;
      const vx = Math.cos(angle) * vel, vy = Math.sin(angle) * vel;
      const s = 8 + Math.random() * 6, dur = 500 + Math.random() * 300;
      ...
      sc.closePath(); sc.fillStyle = `hsl(${hue},100%,65%)`; sc.fill(); sc.restore();
      document.body.appendChild(star);
      const t0 = performance.now(), initL = rect.left+x-s*2, initT = rect.top+y-s*2;
      let rot = Math.random() * 360; const rotSpeed = (Math.random()-0.5)*720;
      (function tick(now: number) {
        const elapsed = now-t0, t = Math.min(elapsed/dur, 1);
        star.style.left = `${initL + vx*elapsed*0.001*60}px`; star.style.top = `${initT + vy*elapsed*0.001*60}px`;
        rot += rotSpeed/60; star.style.transform = `rotate(${rot}deg) scale(${1-t*0.4})`; star.style.opacity = String(1-t*t);
  ...
  const onStart = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const pos = getPos(e, canvasRef.current!); points.current = [pos]; spawnStars(canvasRef.current!, pos.x, pos.y);
  };
  const onMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (Math.random() < 0.3) spawnStars(canvasRef.current!, pos.x, pos.y);
  };
...
export default function ReviewRelearn({ discoveredLetters, allLetters, onComplete, dayNumber }: ReviewRelearnProps) {
  ...
  const stopPanuto = useCallback(() => { if (panutoRef.current) { panutoRef.current.pause(); panutoRef.current.currentTime = 0; panutoRef.current = null; } }, []);
  useEffect(() => () => { stopPanuto(); }, []);

  // Build one round per discovered letter, shuffled — done once on mount
  const [queue] = useState<ReviewRound[]>(() => buildAllRounds(discoveredLetters, allLetters));
  ...
  useEffect(() => { const audio = new Audio('/Panuto_3.mp3'); panutoRef.current = audio; audio.play().catch(() => {}); }, []);
      ...
      <div style={{ width: '100%', height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div className="text-center flex-shrink-0" style={{ paddingTop: 8, paddingBottom: 4 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'linear-gradient(135deg,#fbbf24,#f59e0b)', borderRadius: 999, padding: '4px 14px', marginBottom: 6, boxShadow: '0 2px 8px rgba(245,158,11,0.35)' }}>
            ...
            <span className="font-fredoka font-bold text-white" style={{ fontSize: 'clamp(12px, 2.5vmin, 16px)' }}>Review at Relearn — Araw {dayNumber}</span>
          </div>
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', paddingLeft: 'clamp(8px,5vw,48px)', paddingRight: 'clamp(8px,5vw,48px)', paddingBottom: 'clamp(6px,2vh,12px)' }}>
          <div style={{ width: '100%', maxWidth: 560, height: '100%', maxHeight: '44dvh', borderRadius: 28, overflow: 'hidden', background: 'rgba(255,255,255,0.85)', boxShadow: '0 8px 32px rgba(0,0,0,0.13)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
